...
- `perturbation.rs` — deep-zoom path past the `f64` wall (~1e-15). `ReferenceOrbit` computes one reference orbit (`from_center_f64`, or `from_center_decimal` in arbitrary precision via `dashu`), and `mandelbrot_perturbation` iterates every pixel as a small `f64` delta off it (`δ_{n+1} = 2·Z_n·δ_n + δ_n² + dc`), emitting the same `Pixel`. Scalar for now (SIMD is a later optimization); no rebasing/BLA yet, so a pixel can only iterate as far as the reference orbit's length. See it via `render_png --deep`.
...
- `render_pods.rs` — `FULLSCREEN_QUAD_VERTS` (the quad is generated vertex-buffer-free in the shaders) and `DrawParams` (proj matrix + texture size), pushed as **immediates** (push constants). `DrawParams` derives `encase::ShaderType` for the GPU layout (glam values bridge through `mint`, since `encase` has no glam-0.33 support yet).
