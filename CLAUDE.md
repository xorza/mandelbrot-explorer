...
A desktop Mandelbrot explorer (crate name `fractal`). The set is computed on the **CPU** with `portable_simd` across all physical cores; the **GPU** (wgpu) only uploads the iteration-count texture, maps it through a palette, and does a cheap reprojection trick to give an instant preview while new tiles compute. Interactive pan/zoom.
...
- Test: `cargo test` (tests are debug; the one in `mandelbrot_simd` self-times via `#[cfg(test)]` blocks). Single test: `cargo test mandelbrot` (the only test lives in `mandelbrot_simd.rs`).
- Standard verification: `cargo fmt --all && cargo check && cargo clippy --all-targets -- -D warnings`.

`palette.png` is loaded at runtime from the crate root (used as the 1-D palette texture), so the working directory matters when running.
...
- `buffer_pool.rs` — recycling allocator for tile pixel buffers. `BufferHandle` returns its `Vec<u8>` to the pool on `Drop` (via a `Weak` back-ref), avoiding per-tile reallocation.
...
2. Tiles are sorted by distance from `focus` so the area the user is looking at computes **first**.
3. Each in-view tile that needs work spawns a tokio task. Concurrency is throttled by a `Semaphore` sized to `physical_cores * 2`. Out-of-view tiles, and tiles being recomputed, have their existing task cancelled via the `AtomicBool` token. Completed buffers move to `TileState::WaitForUpload`; the task fires `tile_ready_callback`, which posts `UserEvent::TileReady` through the winit `EventLoopProxy` to request a redraw.
